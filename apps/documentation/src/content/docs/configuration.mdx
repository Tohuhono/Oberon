...
OberonCMS starter apps are configured in two files under `oberon/`:

- `adapter.ts` for Oberon runtime composition
- `config.tsx` for runtime config and Puck component configuration
...
`config.tsx` is the runtime config for your Puck components. Keep Oberon docs brief and use Puck as
the primary reference for overlapping API surface.
