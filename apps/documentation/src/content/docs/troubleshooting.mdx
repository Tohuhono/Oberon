...
Starter app names must be valid npm package names (lowercase, hyphens OK).
...
async function getDynamicStylesheets() {
  const activeHash = getActiveHash(await adapter.getSetting("@oberoncms/plugin-tailwind", "state"))

  return activeHash ? [`/cms/api/tailwind/${encodeURIComponent(activeHash)}.css`] : []
}
...
1. **Check the [GitHub discussions](https://github.com/Tohuhono/Oberon/discussions)**
2. **Search [GitHub issues](https://github.com/Tohuhono/Oberon/issues)** for similar problems
