...
The development plugin helps you run OberonCMS quickly without provisioning external infrastructure
...
<Callout type="info">
  In production, you should use real database and send plugins (for example, Turso/PostgreSQL plus a
...
For local development, verification requests are logged instead of sent by email. This allows quick
sign-in testing without SMTP setup.
