...
OberonCMS uses an auth plugin for sign-in, session handling, and role-aware access control.
...
Add the auth plugin to your server-side Oberon config in `adapter.ts`.
...
The current auth flow does not require auth-specific database adapter methods. It requires a send
adapter that implements `sendVerificationRequest` so codes can be delivered to users.
...
In development mode, verification details are logged by the development plugin instead of being
sent.
