...
Apps now define one server-only config with `defineConfig({ client, plugins })` in `adapter.ts` and
pass it directly to `initOberon(config)` and `bootstrapOberon(config)`. Bootstrap lifecycle work has
moved from the public adapter `prebuild` method to top-level plugin `bootstrap(next)` hooks, and the
public Adapter no longer exposes `prebuild`.

Generated starter apps now contain `oberon/client.config.tsx` for Puck component configuration and
define server-only plugin composition in `oberon/adapter.ts`. The Next.js recipe output includes the
